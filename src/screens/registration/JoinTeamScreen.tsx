...
export const JoinTeamScreen: React.FC = () => {
  ...
  // Player flow: identity -> (confirm | details). The entry gate resolves the
  // email before any branch starts, and no roster list is ever rendered.
  const [playerFlowStep, setPlayerFlowStep] = useState<'identity' | 'confirm' | 'details'>(
    'identity'
  );
  ...
  /** The footer button renders only on 'team-info', so this has one job. */
  const handleContinue = () => {
    // The entry gate sits between the team card and the role cards.
    goToGate();
  };
...
const styles = StyleSheet.create({
  ...
  /**
   * Long-label escape hatches: the player identity step's, and the parent ceiling
   * escape's "Register as a new player instead". Their labels are far longer than a
   * normal CTA's, and a Text in a row-direction flex parent will not shrink on its
   * own — it ran past the pill edge on narrow screens. flexShrink lets it take the width left by the arrow
